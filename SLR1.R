#simple linear regreession - women

women
head(women)
names(women)
cor(women) #because only two category, this holds good, better to follow line 7
cor(women$height,women$weight) 
cov(women$height,women$weight) #direction of variables
pairs(women)
pairs(mtcars[1:5]) 
?lm() 
...
cbind(new1, p1)
#136.7333 140.1833
#prediction for height 69.5 and 70.5
...
plot(fit1)
<Return>
#prediction for mtcars (wt = 2 & 3)
fit2=lm(mpg~wt,data=mtcars)
mtcarsnew = data.frame(wt=c(2,3))
(mtcarsp = predict(fit2, newdata = mtcarsnew))
cbind(mtcarsnew, mtcarsp)

mtcarsfit = lm(mpg~wt, data= mtcars)
mtcarsnew1 = data.frame(wt=c(2,3))
(mtcarsp1 = predict(mtcarsfit, newdata = mtcarsnew1))
cbind(mtcarsnew1,mtcarsp1)
